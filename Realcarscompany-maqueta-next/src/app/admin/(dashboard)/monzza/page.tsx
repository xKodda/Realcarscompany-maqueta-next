...
async function getSales() {
    try {
        ...
        console.error('Failed to get sales', e);
        return [];
    }
}
                            ...
                            {sales.length === 0 ? (
                                ...
                                sales.map((sale) => (
                                    <tr key={sale.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <div className="font-medium text-[#161b39]">{sale.nombreComprador}</div>
                                            <div className="text-xs text-gray-400">{sale.emailComprador}</div>
                                        </td>
                                        ...
                                        <td className="p-4">
                                            ...
                                            <div className="text-xs text-gray-400 mt-1 truncate max-w-[150px]">
                                                {sale.telefonoComprador || 'Sin teléfono'}
                                            </div>
                                        </td>
                                        <td className="p-4 text-xs text-gray-500 font-medium">
                                            Flow
                                            <div className="mt-1 text-gray-400 bg-gray-100 rounded px-1 py-0.5 inline-block text-[10px]">
                                              {sale.cantidad || 0} Stickers {sale.tickets?.length > 0 && `(Asignados)`}
                                            </div>
                                        </td>
                                        <td className="p-4">
