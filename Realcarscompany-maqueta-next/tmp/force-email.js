...
async function main() {
    const ordenId = 'RC-DUZIJ7RF0';
    console.log('Force updating order:', ordenId);
    ...
    let ticketsIds = order.tickets.map(t => t.numero);
    
    // Si no tiene tickets aún los generamos (si ya lo forzamos no duplica)
    if (ticketsIds.length === 0) {
        ...
        console.log('DB Updated with new tickets! Status:', 'pagado');
    }
    ...
    console.log('Enviando via Resend a:', order.emailComprador);
